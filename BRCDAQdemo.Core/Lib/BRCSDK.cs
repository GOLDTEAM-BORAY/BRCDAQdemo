...
namespace BRCDAQdemo.Core.Lib
{
    ...
    {
#if X64
        private const string DLLPATH = "Lib/x64/libbrcdaqsdk.dll";
#elif X86
        private const string DLLPATH = "Lib/x86/libbrcdaqsdk.dll";
#else
        private const string DLLPATH = "Lib/x64/libbrcdaqsdk.dll";
#endif
        ...
        public static List<ModuleInfo> ScanModules()
        {
            ...
            _moduleInfos = Enumerable.Range(0, moduleCount).Select(index => new ModuleInfo
            {
                ...
                GainOptions = GetModuleInfoGainOptions(index),
        ...
        public static unsafe List<double> GetModuleInfoGainOptions(int index)
        {
            double[] pDouble = new double[1024];
            int length = 0;
            var handle = pDouble.AsMemory().Pin();
            try
            {
                if (get_module_info(index, ModuleInfoType.GainOptions, handle.Pointer, &length) != 0)
                    throw new Exception(GetLastError());
                return new List<double>(pDouble.Take(length));
            }
            finally
        ...
        public static unsafe InterfaceType GetModuleInfoInterfaceType(int index)
        {
            int interfaceType = 0;
            if (get_module_info(index, ModuleInfoType.InterfaceType, &interfaceType, null) != 0)
                throw new Exception(GetLastError());
        ...
        [DllImport(DLLPATH, CallingConvention = CallingConvention.Cdecl)]
        private static extern unsafe int start_via_udp(int* mHandle_array, int length);
        [DllImport(DLLPATH, CallingConvention = CallingConvention.Cdecl)]
        ...
        [DllImport(DLLPATH, CallingConvention = CallingConvention.Cdecl)]
        private static extern int set_calibration(int mHandle, int channel, double sampleRate, double gain, double K, double B);
        [DllImport(DLLPATH, CallingConvention = CallingConvention.Cdecl)]
        private static extern int save_calibration(int mHandle);
        [DllImport(DLLPATH, CallingConvention = CallingConvention.Cdecl)]
        private static extern int reset_calibration(int mHandle);
        [DllImport(DLLPATH, CallingConvention = CallingConvention.Cdecl)]
        private static extern unsafe int check_temperature(int mHandle, float* pChipTemperature, float* pAmbientTemperature);
        ...
        public static unsafe void StartViaUdp(params BrcDevice[] devices)
        {
            if (devices == null || devices.Length == 0)
                throw new ArgumentException("至少需要一个已连接设备", nameof(devices));

            int[] handles = devices.Select(device => device.Handle).ToArray();
            var handle = handles.AsMemory().Pin();
            try
            {
                if (start_via_udp((int*)handle.Pointer, handles.Length) != 0)
                    throw new Exception(GetLastError());
            }
            finally
            {
        ...
        public class ModuleInfo
        {
            ...
            public InterfaceType InterfaceType { get; set; } = InterfaceType.Unknown;
        ...
        public class BrcDevice : IDisposable
        {
            ...
            public void Start(bool rawValue = false)
            {
                if (start(_mHandle, rawValue) != 0)
                    throw new Exception(GetLastError());
            }
            public void Stop()
            {
            ...
            public unsafe void GetChannelsData(Memory<double> array, TimeSpan timeout)
            {
                ...
                try
                {
                    if (get_channels_data(_mHandle, (double*)handle.Pointer, array.Length / _moduleInfo.ChannelCount, array.Length, (int)timeout.TotalMilliseconds) != 0)
                        throw new Exception(GetLastError());
                }
                finally
                {
            ...
            public void SetCalibration(int channel, double sampleRate, double gain, double k, double b)
            {
                if (set_calibration(_mHandle, channel, sampleRate, gain, k, b) != 0)
                    throw new Exception(GetLastError());
            }

            public void SaveCalibration()
            {
                if (save_calibration(_mHandle) != 0)
                    throw new Exception(GetLastError());
            }

            public void ResetCalibration()
            {
                if (reset_calibration(_mHandle) != 0)
                    throw new Exception(GetLastError());
            }

            public unsafe (float ChipTemperature, float AmbientTemperature) CheckTemperature()
            {
                float chipTemperature = 0;
                float ambientTemperature = 0;
                if (check_temperature(_mHandle, &chipTemperature, &ambientTemperature) != 0)
                    throw new Exception(GetLastError());

                return (chipTemperature, ambientTemperature);
            }
